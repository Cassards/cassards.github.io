...
/**
 * Implement a mosaic effect
 */
vec3 mosaic(vec2 coord, int size);
...
void main()
{
	...
	case 5:
		...
		break;
	case 6:
		fragmentColor = vec4(mosaic(gl_FragCoord.xy, mosaicSize), 1.0);
		break;
	case 7:
...
vec3 mosaic(vec2 coord, int size){
    ...
    vec2 referencePixel = vec2(0.0);
    
    referencePixel.x = coord.x - int(coord.x) % size;
    referencePixel.y = coord.y - int(coord.y) % size;
    
    return textureRect(frameBufferTexture, referencePixel).xyz;    
}
